...
| **AL-11** | Forced transfer / seize-and-reallocate | ✅ (code+tests; verify/props pending) | AL-6, AL-9 | **Must-Have** ([AL-9 §3](AL-9-PARITY-GAP-REVIEW.md#al-11--forced-transfer--seize-and-reallocate-must-have)), **delivered.** Court order / lost-key recovery / 17Ad-12 theft recovery. Extends AL-6 forced-*burn* to forced-*reallocation*: a consuming **`SimpleHolding_ForcedSeize`** (controller `admin`) archives the holding (owner's cryptographic proof) + reallocation to a preapproved `newOwner` via `TransferPreapproval_ReceiveSeized` (net supply unchanged). Wrapped in **two-person control** (OQ-2): `Admin_ProposeSeizure` (one `LegalAdmin`) → `SeizureProposal_Approve` (a **different** `LegalAdmin`, `approver /= proposer`). `LegalAdmin` graduated from reserved. **Manual-only**; INV-43/44; suite 158→166 (`Test/Seize.daml`, 7 tests); lint baseline-clean. **Pending:** daml-verify/props rows (branch-pinned, like AL-8/AL-10). v1 scopes unlocked holdings (locked-fund reallocation deferred). |
