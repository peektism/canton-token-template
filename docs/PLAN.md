...
| **AL-11** | Forced transfer / seize-and-reallocate | 📋 (ready; OQs resolved) | AL-6, AL-9 | **Must-Have** ([AL-9 §3](AL-9-PARITY-GAP-REVIEW.md#al-11--forced-transfer--seize-and-reallocate-must-have)). Court order / lost-key recovery / 17Ad-12 theft recovery. Extends AL-6 forced-*burn* to forced-*reallocation* via a **`LegalAdmin`**-gated `ForceTransfer` choice (archives + re-creates holding; archive notification = seizure proof). **Manual-only + two-person control (propose + approve)** — irreversible (OQ-2). |
...
| **2a** | **AL-10** — account freeze + **AL-11** — forced transfer/seize | **AL-10 ✅ delivered + review-hardened** (`frozenAccounts` + `ComplianceAdmin`-gated `Rules_SetAccountFrozen`; unified `assertCanOriginate` gate at all 5 chokepoints + mint + cold-path accept; V2 checks owner **and** provider; INV-40..42; suite 146→158; `LegalAdmin`/`EmergencyOps` reserved). **AL-11 📋 next:** manual-only + **two-person control (propose + approve)** for irreversible seize/forced-transfer; graduates `LegalAdmin` (OQ-2/OQ-4). | AL-9 |
