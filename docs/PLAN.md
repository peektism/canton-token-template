...
|---|---|---|---|
| 1 | **AL-4** — formal-verification model | **PRs open.** Both sub-PRs raised: `daml-verify` capability relation ([OpenZeppelin/daml-verify#4](https://github.com/OpenZeppelin/daml-verify/pull/4): 8 proofs A1–A8, 22/22 proved) and `daml-props` admin-layer rows ([OpenZeppelin/daml-props#2](https://github.com/OpenZeppelin/daml-props/pull/2): `Examples/AdminLayer/`, 5 property tests, fixed + buggy executors). Remaining: merge both, then re-pull the merged tools locally. Pure verification depth, no new on-ledger surface. **Lands in the `$CANTON_TOOLS_HOME` tool repos via branches + PRs** ([§17.6](#176-tooling-workspace-canton_tools_home--contribution-model)). | none |
