...
|---|---|---|---|
| 1 | **AL-7** — decoupled access-control library + branch-based forward dev | **PR open** ([peektism/oz-daml-contracts#1](https://github.com/peektism/oz-daml-contracts/pull/1)): the three independent, token-agnostic packages (`oz-access-control` / `oz-ownable` / `oz-pausable`) are built, tested (16 green), and signed/Verified. **Remaining:** token consumes them via a branch-pinned `data-dependency`, reduces in-token modules to the thin Layer-B composition, and compares against the hybrid baseline before cutover. Full rationale, weighed options, and the Canton size-limit answer in **[ARCHITECTURE.md](ARCHITECTURE.md)**. Built on branches; compared empirically before cutover. | AL-2, AL-3 |
