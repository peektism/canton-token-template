...
| **AL-9** | OZ-parity gap review of the planning docs (US-FI-prioritized) | 📋 (next) | AL-8 | A structured pass over **every doc** ([PLAN §17.4](#174-explicitly-excluded-with-rationale), [ADMIN-LAYER-PLAN §10](ADMIN-LAYER-PLAN.md#10-out-of-scope-for-this-layer), [SCOPE.md](SCOPE.md), [ARCHITECTURE.md](ARCHITECTURE.md)) to find every place we **pre-assume "no motivation" / "MVP / out of scope"** for a feature where we actually want to match OZ Solidity contract-library **breadth**. AL-8 is the exemplar (a "named extension seam" that was really a parity gap). **Decided ([§17.7](#177-resolved-decisions--al-8--al-9)):** AL-9's **first artifact is a US-FI usage-expectations research summary** (OCC/NYDFS, OFAC freeze-seize, BSA/AML allowlist, SOC/audit) that then weights the register; **breadth = OZ `access/` + `governance` *plus* token transfer restrictions** (ERC-1404-style restrictions, freeze/seize, allow/blocklist). Produce a **parity-gap register** classifying each feature by *(OZ-parity coverage) × (US-FI demand) × (DAML feasibility)* and **reclassify "excluded" → prioritized slice** where justified. Review/planning slice — emits prioritized slices, no on-ledger surface itself. |
...
> ⚠️ **Under AL-9 review.** This list mixes two very different kinds of "excluded":
> (a) things that are **impossible or wrong on this ledger** (keyless LF 2.1
> constraints, non-real CIP surfaces) — permanent, and (b) things we **deferred as
> "MVP / extension seam"** that are actually **OZ-parity gaps we do want**, given
> US-FI usage expectations. **AL-9** re-examines every (b) item and reclassifies it
> to a prioritized slice where justified — AL-8 (role-admin hierarchy) is the first
> such reclassification. Each bullet below is tagged **[permanent]** or
> **[AL-9: re-examine]** accordingly.
...
- **[AL-9: re-examine]** **`V2OmnibusAccount`, Memo Pledge, CDP/`SecuritiesIntermediaryRole`/`LiquidatorRole`, `BridgeEscrowRole`** — stablecoin/custody/bridge use-cases; today deferred to sibling repos (correction C7). AL-9 assesses which custody/compliance roles US FIs expect *here* vs. in siblings.
- **[AL-9: re-examine]** **Per-asset emergency *freeze* / account-level seize beyond origination pause** — pause is origination control; a freeze/seize for sanctions/compliance is a separate capability (see [ADMIN-LAYER-PLAN.md §4](ADMIN-LAYER-PLAN.md#4-pausable--origination-control-simpletokenrules)). Forced-burn (AL-6) exists; freeze does not. High US-FI relevance — AL-9 prioritizes.
...
- **[AL-9: re-examine]** **`AccessControlEnumerable` (on-ledger role-member enumeration)** — not expressible keyless (no global lookup); off-ledger indexing covers it. AL-9 confirms the off-ledger contract is sufficient for US-FI audit expectations.
...
| **2** | **AL-9** — OZ-parity gap review of the docs (US-FI-prioritized) | **Next.** Structured pass over all planning docs to find every place we pre-assume "no motivation / MVP / out of scope" for a feature we actually want for OZ-parity breadth. **First artifact: a US-FI usage-expectations research summary** (decided §17.7), which weights a prioritized **parity-gap register** *(OZ coverage × US-FI demand × DAML feasibility)*; **breadth includes token transfer restrictions** (ERC-1404-style/freeze/allowlist). Reclassifies [§17.4](#174-explicitly-excluded-with-rationale) "[AL-9: re-examine]" items into slices. Review/planning slice; emits slices, no on-ledger surface itself. | AL-8 |
