...
| **AL-8** | Full role delegability via a role-admin hierarchy (OZ AccessControl parity) | 🎯 (current) | AL-7 | **Maximally-decoupled, library-first.** Replace the flat binary delegation policy (`delegableRole : Role -> Bool`, today only `Pauser`) so **every role is delegable through its admin role** — enabling the separation-of-duties US FIs expect (treasury vs compliance vs ops). **Decided ([§17.7](#177-resolved-decisions--al-8--al-9)):** the relation is a **compiled pure function** `roleAdmin : Role -> Role` (per-constructor, exhaustiveness-checked; flat default = every role's admin is `Admin`; reconfigured by package upgrade, **not** a runtime `setRoleAdmin` contract). **Library (`oz-access-control`):** mechanism-only generic grant/revoke that require the caller to present a grant for a supplied `adminRole : Text` (consumer computes it from `roleAdmin` via `roleId`), plus `renounceRole` — Text-role genericity preserved, no role graph hard-coded. **Default-admin handoff:** `Admin` stays **root-managed** (keeps C9 safety; not self-administered) with a **two-step + timelocked** handoff (`AccessControlDefaultAdminRules` analogue, composing the `oz-ownable` handshake with a time gate) — a **core** deliverable. **Token (`SimpleToken/Admin`):** consume the library's role-admin-aware grant/revoke; add `renounceRole`. **Verify/props:** new daml-verify rows (granting R requires `roleAdmin(R)`; no privilege escalation; renounce is self-only; timelock cannot be bypassed) + a daml-props role-admin-graph state machine. **Known DAML gap:** `AccessControlEnumerable` (global member enumeration) is not expressible keyless — off-ledger, documented not built. |
...
- **[AL-9: re-examine → AL-8]** **Per-role admin hierarchy (`getRoleAdmin`) / `setRoleAdmin` / two-step / timelocked issuance** — previously "named extension seams; the `Admin` role covers the MVP." **Reclassified: this is an OZ-parity gap we want — now slice AL-8.** On-ledger multisig remains a Canton-topology / sibling concern pending AL-9.
...
`Burner` roles are enforced; only `BatchProcessor` is still reserved. The current
delegation policy is the flat MVP (`delegableRole`, `Pauser`-only) — **AL-8**
graduates it to a full role-admin hierarchy matching OZ breadth.
...
|---|---|---|---|
| **1** | **AL-8** — full role delegability via a role-admin hierarchy (OZ parity) | **Current.** Replace the flat `delegableRole : Role -> Bool` with a **compiled** per-role `roleAdmin : Role -> Role` relation (decided §17.7: pure function, not a runtime `setRoleAdmin` contract) so **every role is delegable through its admin role** — matching OZ `AccessControl` breadth and the separation-of-duties US FIs expect. Library-first/maximally decoupled: `oz-access-control` gains mechanism-only role-admin-aware grant/revoke + `renounceRole`; the token consumes it. `Admin` stays **root-managed** with a **two-step + timelocked** default-admin handoff (core, not candidate). New verify/props rows. Decisions in [§17.7](#177-resolved-decisions--al-8--al-9). | AL-7 |
