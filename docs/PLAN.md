...
- **Role-admin hierarchy (AL-8) — OZ `AccessControl` parity:** implemented and green.
  The flat `delegableRole : Role -> Bool` is replaced by a **compiled** per-role
  `roleAdmin : Role -> Role` relation (flat default: every role's admin is `Admin`),
  so **every role is delegable through its admin role** — an `Admin` delegate may now
  appoint `Minter`/`Burner`/etc., not just `Pauser`. The generic `oz-access-control`
  library gains mechanism-only role-admin-aware grant/revoke (`RoleAdmin_GrantRoleAs`/
  `RevokeRoleAs`, caller supplies the `adminRole : Text`), `renounceRole`
  (`RoleGrant_Renounce`), and a timelocked two-step default-admin handoff
  (`DefaultAdminTransferOffer`); the token consumes it and keeps `Admin` **root-managed**
  (`AccessControlDefaultAdminRules` posture, C9) with a `PendingDefaultAdminTransfer`
  two-step + timelock handoff and a self-only `RoleCapability_Renounce`. See
  [ARCHITECTURE.md §10](ARCHITECTURE.md#10-toward-oz-parity-breadth-slices-al-8--al-9).
  Verify/props landed as branch-pinned sub-PRs: **daml-verify** A9–A13
  (grant-requires-`roleAdmin`, completeness, no escalation, renounce self-only,
  timelock not bypassable — 27/27 proved,
  [#5](https://github.com/OpenZeppelin/daml-verify/pull/5)) and **daml-props**
  `Examples/RoleAdmin/` (5 tests, suite 14,
  [#3](https://github.com/OpenZeppelin/daml-props/pull/3)).
- **Parity-gap review (AL-9) — US-FI-prioritized:** ✅ delivered (independent of AL-8)
  → **[AL-9-PARITY-GAP-REVIEW.md](AL-9-PARITY-GAP-REVIEW.md)**. US-FI usage-expectations
  research summary (OCC/NYDFS, OFAC freeze-seize, BSA/AML, SOC/audit) weights a
  parity-gap register that **emitted 4 prioritized slices** — **AL-10** (account
  freeze), **AL-11** (forced transfer/seize), **AL-12** (modular `TransferValidator`
  restriction), **AL-13** (global halt) — confirmed 4 classified-out, and surfaced
  **6 open questions, now all resolved** (OQ-1..OQ-6, [§17.7](#177-resolved-decisions--al-8--al-9)):
  OQ-2/4/5/6 closed, OQ-1/3 decided-with-posture. **AL-10/AL-11 unblocked** (manual-only
  controls, new `ComplianceAdmin`/`LegalAdmin`/`EmergencyOps` roles, dual-control for
  irreversible seize/forced-transfer); **AL-13 → doc/runbook-only**; **AL-12** single-domain
  v1 pending OQ-1 hybrid-SLA sign-off with compliance + infra. Review/planning slice.
- **Test status:** full suite **146/146 passing** (141 baseline + 5 AL-8 token tests)
  and the `oz-access-control` library **14 AccessControl scripts** (6 baseline + 8 AL-8),
  `scripts/verify.sh` daml-lint **clean on new source** on SDK 3.4.11 / Java 21.
  AL-0..AL-3 and AL-5/AL-6 have reached a clean `/code-review` pass.
...
| **AL-7** | Decoupled access-control library + branch-based forward dev | ✅ (merged) | AL-2, AL-3 | ✅ **Library merged:** three **independent, token-agnostic packages** in `$CANTON_TOOLS_HOME/repos/oz-daml-contracts` — `oz-access-control` (`OpenZeppelin.AccessControl`; `RoleGrant`/`RoleAdmin` + `requireRole`/`hasRole`; Text role ids = Solidity `bytes32` analogue), `oz-ownable` (two-step ownership handshake), `oz-pausable` (`PauseState` + `whenNotPaused`) — each its own DAR, no cross-deps, `daml-script`-free; 16 tests green (`dpm test`); typed-wrapper bridge demoed; every template documents the AGENTS.md checklist ([peektism/oz-daml-contracts#1](https://github.com/peektism/oz-daml-contracts/pull/1), merged). This resolved the review (generic library, not in canton-token; each module independent + minimal). ✅ **Consumption merged** ([peektism/canton-token-template#1](https://github.com/peektism/canton-token-template/pull/1)): token `data-depends` on **only** `oz-access-control`; `requireRole` delegates the admin/assignee/role gates (A1–A3) to `OpenZeppelin.AccessControl.requireRole` via a `RoleGrant` view, keeping the scope gate + the closed `Role` sum bridged by `roleId : Role -> Text`; **full 141-test suite green** (behavioural equivalence; net +43/−15 source lines). **Finding:** scope/allowance stay on `RoleCapability` (Daml has no template extension), pause stays an embedded flag, ownership stays the `Admin` role — all by design; the library's payoff is reuse by *other* consumers. Empirical comparison in **[ARCHITECTURE.md §9](ARCHITECTURE.md)**. |
| **AL-8** | Full role delegability via a role-admin hierarchy (OZ AccessControl parity) | ✅ | AL-7 | **Maximally-decoupled, library-first.** Replace the flat binary delegation policy (`delegableRole : Role -> Bool`, today only `Pauser`) so **every role is delegable through its admin role** — enabling the separation-of-duties US FIs expect (treasury vs compliance vs ops). **Decided ([§17.7](#177-resolved-decisions--al-8--al-9)):** the relation is a **compiled pure function** `roleAdmin : Role -> Role` (per-constructor, exhaustiveness-checked; flat default = every role's admin is `Admin`; reconfigured by package upgrade, **not** a runtime `setRoleAdmin` contract). **✅ Library (`oz-access-control`):** mechanism-only generic grant/revoke (`RoleAdmin_GrantRoleAs`/`RoleAdmin_RevokeRoleAs`) that require the caller to present a grant for a supplied `adminRole : Text` (consumer computes it from `roleAdmin` via `roleId`), plus `renounceRole` (`RoleGrant_Renounce`) — Text-role genericity preserved, no role graph hard-coded; +8 test scripts. **✅ Default-admin handoff:** `Admin` stays **root-managed** (keeps C9 safety; not self-administered) with a **two-step + timelocked** handoff — generic `DefaultAdminTransferOffer` in `oz-access-control` (self-contained: re-expresses the `oz-ownable` two-step shape + a `getTime` gate rather than depending on it, keeping the library packages independent — exactly as OZ's `AccessControlDefaultAdminRules` reimplements its own two-step) consumed by the token's `PendingDefaultAdminTransfer`. **✅ Token (`SimpleToken/Admin`):** `roleAdmin` replaces `delegableRole`; delegated issue/revoke require holding `roleAdmin(targetRole)` (delegating A1–A3 to the library) and refuse `Admin` (DefaultAdminRules posture); self-only `RoleCapability_Renounce`; +5 tests (suite 141→146). **✅ Verify/props (branch-pinned sub-PRs, stacked on the AL-4 branches):** **daml-verify** A9–A13 — granting R requires `roleAdmin(R)`, role-admin grant completeness (non-vacuity), no privilege escalation (root `Admin` never delegable), renounce self-only, timelock not bypassable; `roleAdmin` modeled as an *uninterpreted* function so the proofs hold for every role graph; **27/27 proved, pytest 27/27** ([OpenZeppelin/daml-verify#5](https://github.com/OpenZeppelin/daml-verify/pull/5)). **daml-props** `Examples/RoleAdmin/` — fixed + 3 buggy executors (role-admin auth / root carve-out / renounce self-only), 5 property tests, suite 14 green ([OpenZeppelin/daml-props#3](https://github.com/OpenZeppelin/daml-props/pull/3)). Both signed + **Verified**; retarget to `main` once the AL-4 PRs merge. **Known DAML gap:** `AccessControlEnumerable` (global member enumeration) is not expressible keyless — off-ledger, documented not built. |
| **AL-9** | OZ-parity gap review of the planning docs (US-FI-prioritized) | ✅ (review delivered; emits AL-10..AL-13 + 6 OQs) | none (independent of AL-8) | **Delivered → [AL-9-PARITY-GAP-REVIEW.md](AL-9-PARITY-GAP-REVIEW.md).** A structured pass over **every doc** ([PLAN §17.4](#174-explicitly-excluded-with-rationale), [ADMIN-LAYER-PLAN §10](ADMIN-LAYER-PLAN.md#10-out-of-scope-named-seams-not-gaps), [SCOPE.md](SCOPE.md), [ARCHITECTURE.md](ARCHITECTURE.md)) to find every place we **pre-assumed "no motivation" / "MVP / out of scope"** for a feature where we actually want OZ Solidity contract-library **breadth**. AL-8 is the exemplar (a "named extension seam" that was really a parity gap). **Per [§17.7](#177-resolved-decisions--al-8--al-9):** the **first artifact is a US-FI usage-expectations research summary** ([§1](AL-9-PARITY-GAP-REVIEW.md#1-us-fi-usage-expectations-research-summary-tt-q-009a-artifact): OCC/NYDFS, OFAC freeze-seize, BSA/AML allowlist, SOC/audit) that weights a **parity-gap register** ([§2](AL-9-PARITY-GAP-REVIEW.md#2-parity-gap--prioritization-register)) scored *(OZ coverage) × (US-FI demand) × (DAML feasibility)*; **breadth = OZ `access/` + `governance` *plus* token transfer restrictions** (ERC-1404/3643, freeze/seize, allow/blocklist). **Outcome:** reclassified the [§17.4](#174-explicitly-excluded-with-rationale) "[AL-9: re-examine]" items → **3 Must-Have slices (AL-10/11/12) + 1 Strong slice (AL-13)**; confirmed 4 classified-out (`Enumerable` off-ledger, `Governor`/`Votes` off-ledger, `Ownable` anti-pattern, custody/CDP/bridge → siblings). **6 open questions surfaced and resolved (OQ-1..OQ-6 / TT-Q-009c..009h, 2026-06-07):** OQ-2/4/5/6 closed, OQ-1/3 decided-with-posture — AL-10/AL-11 unblocked (manual-only, dual-control for seize), AL-13 → runbook-only, AL-12 single-domain v1 pending OQ-1 SLA sign-off. Review/planning slice — no on-ledger surface itself. |
| **AL-10** | Account-level freeze / compliance hold | ✅ | AL-2, AL-9 | **Must-Have** ([AL-9 §3](AL-9-PARITY-GAP-REVIEW.md#al-10--account-level-freeze--compliance-hold-must-have)), **delivered + review-hardened.** OFAC/AML/GENIUS-Act per-account compliance hold, distinct from AL-1 origination pause. **`frozenAccounts : [Party]` on `SimpleTokenRules`** (same non-spoofable shape as `paused`), maintained by registry-wide **`ComplianceAdmin`**-gated **`Rules_SetAccountFrozen`** (a `Bool` freeze/unfreeze mirroring `Rules_SetPaused`; redundant-change rejected; admin never freezable) + `Rules_GetFrozenAccounts` read. Pause + freeze are unified into one **`assertCanOriginate`** gate at **all five origination chokepoints** (V1/V2 transfer, V1/V2 allocation, batch settlement) **+ direct mint + cold-path mint accept** (`Rules_AcceptMintProposal`, factory-mediated so the freeze is re-checked at holding-creation). V2 guards check **`accountParties`** (owner **and** provider — freezing either blocks a provider-managed flow). New **`ComplianceAdmin`** role (`roleAdmin = Admin`, delegable); `LegalAdmin`/`EmergencyOps` added **reserved** (AL-11/AL-13). **Manual-only, single officer** (reversible; OQ-2); `reason` recorded for audit. Freeze is origination control like pause — committed inbound flows still complete (seizure = AL-11). **Suite 146→158** (12 `Test/Freeze.daml` incl. V2 basic, provider-managed, mint-proposal-accept); INV-40..42; lint baseline-clean. Review fixes #1–#8 in `c2e61f3`/`f062f16`. |
| **AL-11** | Forced transfer / seize-and-reallocate | ✅ (code+tests; verify/props pending) | AL-6, AL-9 | **Must-Have** ([AL-9 §3](AL-9-PARITY-GAP-REVIEW.md#al-11--forced-transfer--seize-and-reallocate-must-have)), **delivered.** Court order / lost-key recovery / 17Ad-12 theft recovery. Extends AL-6 forced-*burn* to forced-*reallocation*: a consuming **`SimpleHolding_ForcedSeize`** (controller `admin`) archives the holding (owner's cryptographic proof) + reallocation to a preapproved `newOwner` via `TransferPreapproval_ReceiveSeized` (net supply unchanged). Wrapped in **two-person control** (OQ-2): `Admin_ProposeSeizure` (one `LegalAdmin`) → `SeizureProposal_Approve` (a **different** `LegalAdmin`, `approver /= proposer`, **neither officer the beneficiary** — separation of duties). `LegalAdmin` graduated from reserved. **Manual-only**; INV-43/44; suite 158→170 (`Test/Seize.daml`, 11 tests incl. beneficiary/cross-registry/scoped); review-hardened (`/code-review`); lint baseline-clean. **Pending:** daml-verify/props rows (branch-pinned, like AL-8/AL-10). **3 open design calls** ([§17.7](#177-resolved-decisions--al-8--al-9)): locked-fund seizure, structural-vs-convention two-person, court-order onboarding. |
| **AL-12** | Modular transfer-restriction hook (ERC-1404/3643) | 📋 (scoped; confirm OQ-1 SLA first) | AL-2, AL-9 | **Must-Have/Strong** ([AL-9 §3](AL-9-PARITY-GAP-REVIEW.md#al-12--modular-transfer-restriction-hook-must-have--strong)). FinCEN/KYC/Reg-D-S. Pluggable `TransferValidator` interface (`BlocklistValidator`/`ClaimsValidator`/jurisdiction) via Daml Finance polymorphism. **Hybrid validation** (cached routine + live high-risk; redemption stays available; OQ-1) — ⚠️ confirm SLA/failover with compliance + infra first. **Single-domain v1, issuer-held claims; cross-domain identity deferred** (OQ-3). `ComplianceAdmin` maintains rule sets. |
| **AL-13** | Global halt — incident-response runbook | 📋 (doc/runbook-only, OQ-5) | AL-1, AL-10 | **Strong** ([AL-9 §3](AL-9-PARITY-GAP-REVIEW.md#al-13--reference-contract-global-halt-strong)). **Resolved doc/runbook-first (OQ-5):** AL-1 (origination pause) + AL-10 (account freeze) + sync-domain suspension (break-glass) cover NYDFS redemption-pause / incident response — **no reference-contract on the transfer hot path** in v1. `EmergencyOps` holds pause/halt authority. Revisit only if per-instrument on-ledger halt is later requested. |
...
> This list mixes two kinds of "excluded": (a) things that are **impossible or wrong
> on this ledger** (keyless LF 2.1 constraints, non-real CIP surfaces) — permanent,
> and (b) things we **deferred as "MVP / extension seam"** that are actually
> **OZ-parity gaps we do want**, given US-FI usage expectations. AL-9 re-examined
> every (b) item and **reclassified** it: the freeze/seize/transfer-restriction gaps
> became prioritized slices **AL-10/11/12** (Must-Have) and **AL-13** (Strong); the
> rest were **confirmed classified-out** (off-ledger / sibling / anti-pattern). Each
> bullet below is tagged **[permanent]**, **[✅ AL-N — delivered/slated]**, or
> **[confirmed classified-out]** accordingly.

Per [ADMIN-LAYER-PLAN.md §10](ADMIN-LAYER-PLAN.md#10-out-of-scope-for-this-layer)
and the research corrections:

- **[permanent]** **Keyed `GlobalPause` / `TokenAdministrator` / `RoleCapability`** — impossible on LF 2.1 (corrections C1/C2/C3).
- **[permanent]** **On-ledger transfer of the instrument `admin` party (a transferable `owner` field)** — the admin party is baked into every `InstrumentId`/holding, so it cannot move without re-issuing assets (correction C9). Ownership is the **`Admin` role**: handoff = grant/revoke the `Admin` capability. Operator key handoff is a Canton topology operation. *(Note: a two-step/timelocked **default-admin** handoff à la `AccessControlDefaultAdminRules` — distinct from moving the immutable party — is an AL-9 parity candidate.)*
- **[permanent]** **`V2BatchTransferFactory` (foldlA array dispatch)** — not the real CIP-112 surface (correction C5).
- **[confirmed classified-out — siblings]** **`V2OmnibusAccount`, Memo Pledge, CDP/`SecuritiesIntermediaryRole`/`LiquidatorRole`, `BridgeEscrowRole`** — stablecoin/custody/bridge use-cases; deferred to sibling repos (correction C7). **AL-9 ([§3 R10](AL-9-PARITY-GAP-REVIEW.md#confirmed-classified-out-not-reopened)) confirms these stay in siblings** — only the **compliance & legal admin roles** (`ComplianceAdmin`/`LegalAdmin`/`EmergencyOps`) the freeze/seize slices need belong *here* (OQ-4).
- **[✅ AL-9 → slated AL-10/AL-11/AL-13]** **Per-asset emergency *freeze* / account-level seize beyond origination pause** — pause is origination control; freeze/seize for sanctions/compliance is a separate capability (see [ADMIN-LAYER-PLAN.md §4](ADMIN-LAYER-PLAN.md#4-pausable--origination-control-simpletokenrules)). Forced-*burn* (AL-6) exists. **AL-9 reclassified this as a Must-Have parity gap and split it into emitted slices: [AL-10](AL-9-PARITY-GAP-REVIEW.md#al-10--account-level-freeze--compliance-hold-must-have) (account freeze), [AL-11](AL-9-PARITY-GAP-REVIEW.md#al-11--forced-transfer--seize-and-reallocate-must-have) (forced transfer/seize-reallocate), [AL-13](AL-9-PARITY-GAP-REVIEW.md#al-13--reference-contract-global-halt-strong) (global halt)** — AL-9's open questions now resolved (manual-only, dual-control for irreversible seize, AL-13 runbook-only; [§17.7](#177-resolved-decisions--al-8--al-9)).
- **[✅ AL-8 — delivered]** **Per-role admin hierarchy (`getRoleAdmin`) / two-step / timelocked issuance** — was "named extension seams; the `Admin` role covers the MVP," reclassified as an OZ-parity gap and **built in AL-8**: compiled `roleAdmin : Role -> Role` (every role delegable through its admin role) + a two-step + timelocked default-admin handoff. Note `setRoleAdmin` is deliberately **not** a runtime contract (TT-Q-008a: reconfiguration is a package upgrade). On-ledger multisig remains a Canton-topology / sibling concern pending AL-9.
- **[confirmed classified-out — off-ledger]** **`AccessControlEnumerable` (on-ledger role-member enumeration)** — not expressible keyless (no global lookup); off-ledger indexing of active role-grant contracts covers it. **AL-9 ([§3 R7](AL-9-PARITY-GAP-REVIEW.md#confirmed-classified-out-not-reopened)) confirms** the off-ledger contract is sufficient for SOC-2 audit, pending stakeholder sign-off (OQ-6).
- **[confirmed classified-out — off-ledger]** **On-chain `Governor`/`ERC20Votes` governance** — US-FIs govern off-ledger (board resolutions) and execute via the AL-8 multi-step admin; *Nice-to-Have*, not slated ([AL-9 §2 R8](AL-9-PARITY-GAP-REVIEW.md#2-parity-gap--prioritization-register)).
- **[✅ AL-9 → slated AL-12]** **Modular transfer restrictions (ERC-1404/3643 allow/blocklist, KYC, jurisdiction gating)** — absent today; **AL-9 reclassified as Must-Have/Strong** → [AL-12](AL-9-PARITY-GAP-REVIEW.md#al-12--modular-transfer-restriction-hook-must-have--strong) (pluggable `TransferValidator` interface), OQ-1/OQ-3-gated.

### 17.5 Current status & next steps (roadmap)

**Done & converged** (clean `/code-review`, 141/141, `verify.sh` 3/3): the full
admin layer **AL-0..AL-3**, the supply layer **AL-5/AL-6**, V2 pause coverage
**V2-9**, and the decoupled library **AL-7** (merged both repos). The `Minter`/
`Burner` roles are enforced; only `BatchProcessor` is still reserved. **AL-8 has
graduated the flat MVP delegation policy (`delegableRole`, `Pauser`-only) to a full
role-admin hierarchy matching OZ breadth** — `roleAdmin : Role -> Role` (every role
delegable through its admin role), a generic role-admin-aware grant/revoke +
`renounceRole` + timelocked two-step default-admin handoff in `oz-access-control`,
consumed by the token (suite 146/146, library +8 scripts). Its verify/props rows
landed as branch-pinned sub-PRs (daml-verify A9–A13, 27/27 proved; daml-props
`Examples/RoleAdmin/`, suite 14).
...
|---|---|---|---|
| **1** | **AL-8** — full role delegability via a role-admin hierarchy (OZ parity) | **✅ Delivered (green).** Replaced the flat `delegableRole : Role -> Bool` with a **compiled** per-role `roleAdmin : Role -> Role` relation (decided §17.7: pure function, not a runtime `setRoleAdmin` contract) so **every role is delegable through its admin role**. `oz-access-control` gained mechanism-only role-admin-aware grant/revoke (`RoleAdmin_GrantRoleAs`/`RevokeRoleAs`) + `renounceRole` (`RoleGrant_Renounce`) + a self-contained timelocked two-step default-admin handoff (`DefaultAdminTransferOffer`); the token consumes them, keeps `Admin` **root-managed** (`PendingDefaultAdminTransfer` handoff; `AccessControlDefaultAdminRules` posture, C9) and adds self-only `RoleCapability_Renounce`. Suite 146/146; library +8 scripts; lint clean on new source. **Verify/props** landed as branch-pinned sub-PRs: daml-verify A9–A13 (27/27 proved, [#5](https://github.com/OpenZeppelin/daml-verify/pull/5)) + daml-props `Examples/RoleAdmin/` (5 tests, suite 14, [#3](https://github.com/OpenZeppelin/daml-props/pull/3)). Decisions in [§17.7](#177-resolved-decisions--al-8--al-9). | AL-7 |
| **2** | **AL-9** — OZ-parity gap review of the docs (US-FI-prioritized) | **✅ Delivered → [AL-9-PARITY-GAP-REVIEW.md](AL-9-PARITY-GAP-REVIEW.md)** (independent of AL-8). Structured pass over all planning docs producing the US-FI usage-expectations research summary + weighted parity-gap register *(OZ coverage × US-FI demand × DAML feasibility)*. **Emitted 4 prioritized slices** (AL-10/11/12 Must-Have, AL-13 Strong), confirmed 4 classified-out, and **surfaced 6 open questions (OQ-1..OQ-6)** that gate the emitted slices. Review/planning slice — no on-ledger surface itself. | none |
| **2a** | **AL-10** — account freeze + **AL-11** — forced transfer/seize | **Both ✅ delivered.** AL-10: `frozenAccounts` + `ComplianceAdmin`-gated `Rules_SetAccountFrozen`; unified `assertCanOriginate` gate; V2 owner+provider; INV-40..42; formally verified (daml-verify#6 / daml-props#4). AL-11: two-person `SeizureProposal` (`Admin_ProposeSeizure` → `SeizureProposal_Approve`, `approver /= proposer`), `SimpleHolding_ForcedSeize` + reallocation via preapproval; `LegalAdmin` graduated; INV-43/44; suite →166. **AL-11 verify/props rows pending** (branch-pinned). | AL-9 |
| **2b** | **AL-12** — modular `TransferValidator` restriction hook | **📋 Scoped: single-domain v1, hybrid validation.** ⚠️ Confirm the OQ-1 hybrid topology + SLA/failover with **compliance + infra** before building; cross-domain identity deferred (OQ-3). | AL-9 + OQ-1 stakeholder sign-off |
| **2c** | **AL-13** — global halt | **📋 Doc/runbook-only (OQ-5).** Write the incident-response runbook (AL-1 + AL-10 + sync-domain suspension); **no new hot-path code** unless per-instrument on-ledger halt is later requested. | AL-1, AL-10 |
| 3 | **AL-4** — formal-verification model (**branch-pinned, do not block on merge**) | Both sub-PRs are open and **Verified**: `daml-verify` A1–A8 ([#4](https://github.com/OpenZeppelin/daml-verify/pull/4): 22/22 proved) and `daml-props` `Examples/AdminLayer/` ([#2](https://github.com/OpenZeppelin/daml-props/pull/2): 5 property tests). **Develop/test forward on the PR branches** — point `scripts/setup.sh` / local verify at the branch refs now. Upstream merge is **downstream of our own validation, not a precondition**; re-pin to `main` once each merges. AL-8 adds new rows here. | none |
| 4 | **V2-10** — `BatchProcessor`-gated `SettlementFactory_SettleBatch` | Graduates the last reserved role: a delegated matching engine drives batch settlement on behalf of executors via a `BatchProcessor` capability (the real CIP-112 surface, replacing the research's invented `V2BatchTransferFactory`). With AL-8, `BatchProcessor` delegability falls out of its `roleAdmin`. | AL-2 |
| 5 | **V2 provider-managed clawback** (V2 hardening) | The documented V1-only gap: add forced-burn to `ProviderManagedSimpleHolding`/`ProviderManagedLockedHolding` for parity with AL-6, or keep deferred. | V2 source-of-record |
| 6 | **V2-11 … V2-16** — CIP-112 conformance | V1/V2-compatible allocations, receipt allocations, reduced-privacy observer mode, view-count baselines, broader transfer events, and the full §5.4/§5.5 conformance matrix (per [CIP-0112-EXTENSION-PLAN.md](CIP-0112-EXTENSION-PLAN.md)). | preview→accepted DAR gate |
...
### 17.7 Resolved decisions — AL-8 / AL-9

Confirmed 2026-06-07. These bind the AL-8/AL-9 design (recorded so they are not
silently re-litigated).

**AL-8 (role-admin hierarchy):**

- **TT-Q-008a — Role-admin relation: ✅ compiled pure function.** Use
  `roleAdmin : Role -> Role` (token) recompiled to change — simplest, safest,
  exhaustiveness-checked under `-Werror=incomplete-patterns`. **Not** runtime-mutable:
  reconfiguring the hierarchy is a deliberate package upgrade, not a live `setRoleAdmin`
  transaction (no `RoleAdminConfig` contract, nothing extra to disclose). The generic
  `oz-access-control` side stays mechanism-only: its grant/revoke require the caller to
  present a grant for a supplied `adminRole : Text`, and the consumer computes that
  `Text` from its compiled `roleAdmin` via `roleId` — so genericity is preserved
  without the library hard-coding any role graph.
- **TT-Q-008b — Admin posture: ✅ root-managed + two-step/timelock handoff.** `Admin`/
  `DEFAULT_ADMIN_ROLE` is **not** self-administered (we keep the C9 safety: no free
  re-delegation). Instead AL-8 ships an `AccessControlDefaultAdminRules`-style
  **two-step + timelocked** default-admin handoff (composing the `oz-ownable` handshake
  primitive with a time gate) — now a **core** AL-8 deliverable, not a candidate.
- **TT-Q-008c — Default hierarchy shape: ✅ flat default.** Every role's admin defaults
  to `Admin`; intermediate admin roles (e.g. `MinterAdmin`) are opt-in by editing the
  compiled `roleAdmin` in a later change, not pre-wired.

**AL-9 (parity-gap review):**

- **TT-Q-009a — US-FI signal: ✅ AL-9 produces the research first.** AL-9's first
  artifact is a US-FI usage-expectations summary (regulatory drivers: OCC/NYDFS,
  OFAC sanctions freeze-seize, BSA/AML allowlist, SOC/audit enumeration) that then
  weights the parity-gap register.
- **TT-Q-009b — Breadth ceiling: ✅ access + governance core *plus* token transfer
  restrictions.** Target OZ `access/` (AccessControl, Ownable, Pausable) + `governance`
  (timelock/votes) **and** token-extension parity OZ ships adjacently — ERC-1404-style
  transfer restrictions, freeze/seize, allow/blocklist (all high US-FI relevance).

**AL-9 review outcome (delivered → [AL-9-PARITY-GAP-REVIEW.md](AL-9-PARITY-GAP-REVIEW.md)):**
the parity-gap register emitted **4 prioritized slices** — **AL-10** (account-level
freeze, Must-Have), **AL-11** (forced transfer / seize-reallocate, Must-Have),
**AL-12** (modular `TransferValidator` restriction hook, Must-Have/Strong), **AL-13**
(reference-contract global halt, Strong) — and **confirmed 4 classified-out**
(`AccessControlEnumerable` + `Governor`/`Votes` are off-ledger; `Ownable` single-key
is an anti-pattern; custody/CDP/bridge roles stay in siblings).

**AL-9 open questions — resolved (2026-06-07).** OQ-2/4/5/6 **closed**; OQ-1/3
**decided with a defaulted posture, flagged for compliance + infra validation before
AL-12**. Net RI posture: *safe by default; manual for deprivation actions; dual-control
for irreversible legal actions; no unnecessary hot-path contracts; no
global-enumerability hacks; single-domain v1; hybrid compliance validation; cross-domain
identity deferred.* Full context in [AL-9-PARITY-GAP-REVIEW.md §4](AL-9-PARITY-GAP-REVIEW.md#4-open-questions--resolved-2026-06-07).

- **TT-Q-009c (OQ-1) — Compliance-validation topology: ✅ Hybrid (validate w/ stakeholders).**
  *Gates AL-12.* Cached allowlist/rule-set for routine transfers; **live issuer validation
  for high-risk** (large value / new counterparty / restricted jurisdiction / suspicious /
  policy override). **Redemption/burn stays available even when ordinary transfers are
  halted**; document SLA + failover. ⚠️ Confirm topology/SLA with compliance + infra before AL-12.
- **TT-Q-009d (OQ-2) — Freeze/seize automation: ✅ Manual-only. CLOSED.** Never auto-fire from
  oracle/expiry/timer/rule-failure. **Freeze/unfreeze: single `ComplianceAdmin`** (reversible,
  speed). **Seize / forced transfer / clawback: two-person control (propose + approve)**
  (irreversible). Every action emits audit metadata (reason/reference).
- **TT-Q-009e (OQ-3) — Cross-sync-domain identity: ✅ Single-domain v1.** AL-12 v1 = issuer-known
  parties + issuer-held claims; **cross-domain identity explicitly deferred** to a future slice
  (trust frameworks / ZK / attestations / cross-domain oracle), not a v1 requirement.
- **TT-Q-009f (OQ-4) — Role placement: ✅ Add the three generic roles here. CLOSED.**
  `ComplianceAdmin` (freeze/unfreeze + transfer rules), `LegalAdmin` (seize / forced transfer /
  court-order; **distinct** from `ComplianceAdmin` for SoD), `EmergencyOps` (pause / emergency
  halt) enter the template `Role` enum. **Flat `roleAdmin _ = Admin`** (per TT-Q-008c); each new
  constructor adds `roleAdmin`/`roleId` under `-Werror=incomplete-patterns`. Custody/CDP/bridge
  roles stay in siblings.
- **TT-Q-009g (OQ-5) — Global-halt mechanism: ✅ Runbook/doc-first. CLOSED.** AL-1 (origination
  pause) + AL-10 (account freeze) + sync-domain suspension (break-glass) suffice. **No
  reference-contract on the transfer hot path** in v1. **AL-13 v1 = incident-response runbook.**
  Revisit only if per-instrument on-ledger halt is explicitly requested.
- **TT-Q-009h (OQ-6) — Off-ledger enumeration: ✅ Acceptable. CLOSED.** Grant/revoke events are the
  authoritative audit trail; an off-ledger index materializes current membership; auditors get
  observer/query access. Document the index as the SOC/audit control; do not mimic
  `AccessControlEnumerable` on-ledger.

**AL-11 open design calls — ⏳ awaiting decision** (surfaced by the `/code-review` of the
AL-11 seizure; the code is delivered and review-hardened, these shape the *next* iteration,
not blockers). See [ADMIN-LAYER-PLAN §11.4](ADMIN-LAYER-PLAN.md#114-accepted-limitations-surfaced-not-hidden).

- **TT-Q-011a — ⏳ Locked/in-flight seizure.** AL-11 v1 seizes only unlocked `SimpleHolding`
  (type-scoped). A target can shield funds from value-preserving *reallocation* by keeping them
  locked/in-flight, leaving only the destructive `LockedSimpleHolding_ForcedBurn`. **Decide:** add
  a locked seize-and-reallocate path (more surface: every wrapper type), or accept that locked
  funds can only be force-*burned* (the AL-6 posture) and document it. *(Recommendation: accept
  for v1 — burn-of-locked already closes the "shield by locking" hole against value escape; pure
  reallocation of locked funds is a narrow add.)*
- **TT-Q-011b — ⏳ Two-person control: structural vs convention.** Separation of duties lives in the
  `SeizureProposal` wrapper; `SimpleHolding_ForcedSeize` is `controller admin` with no link to it
  (it can't reference `SeizureProposal` without a module cycle), so a future *admin-signed* choice
  calling `ForcedSeize` directly would bypass two-person control. A lone capability holder cannot
  reach it (only root / admin-signed contracts can). **Decide:** accept "by convention + gated to
  root, documented" (the current `MintInto`/`ForcedBurn`-style posture), or invest in a structural
  link (e.g. a capability-evidence token the holding choice consumes). *(Recommendation: accept +
  document the obligation on new admin-signed choices; a structural link fights the keyless/no-cycle
  constraints for marginal gain over the root-only reachability.)*
- **TT-Q-011c — ⏳ Reallocation destination must be onboarded.** Reallocation needs `newOwner` to
  hold a `TransferPreapproval` (creating a holding needs the recipient's authority), so a court
  order naming an un-onboarded custodian requires onboarding first. **Decide:** accept (operators
  onboard the custodian, or seize into a standing recovery account), or add a "pending seized claim
  the custodian later accepts" flow (a third step). *(Recommendation: accept — it is the same
  requirement as any transfer recipient; a claim-accept flow adds latency to a court-ordered action.)*
