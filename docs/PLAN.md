...
| **AL-7** | Decoupled access-control library + branch-based forward dev | 🎯 (next) | AL-2, AL-3 | Extract the **generic** substrate to `$CANTON_TOOLS_HOME/repos/oz-daml-contracts` as **three independent, token-agnostic packages** — `OpenZeppelin.AccessControl` (Text role ids, mirroring Solidity `bytes32`), `OpenZeppelin.Ownable`, `OpenZeppelin.Pausable` — each importable on its own (resolves the review: generic library, not in canton-token; each module independent, minimal dependency footprint). Token-template consumes them via a **branch-pinned `data-dependency`** and reduces its in-token modules to the thin **Layer-B composition** (`Admin → owner`, `Pauser`-gated pause, closed `Role` sum via a typed wrapper). Explores the DAML genericity tradeoff (monomorphic templates ⇒ Text-role vs closed-sum) **in isolation, in parallel** with the tested hybrid baseline, then compares before any cutover. Full rationale + options weighed + Canton size-limit answer in **[ARCHITECTURE.md](ARCHITECTURE.md)**. Worked on branches; **does not block on external review/merge** ([§17.6](#176-tooling-workspace-canton_tools_home--contribution-model)). |
...
|---|---|---|---|
| 1 | **AL-7** — decoupled access-control library + branch-based forward dev | **Next.** Acts on the review: extract the generic RBAC/Ownable/Pausable substrate into `oz-daml-contracts` as three **independent, token-agnostic packages**, consumed by the token via a **branch-pinned `data-dependency`**, leaving only the thin Layer-B composition in-token. Explores the Text-role-vs-closed-sum genericity tradeoff **in parallel** with the tested hybrid baseline. Full rationale, weighed options, and the Canton size-limit answer in **[ARCHITECTURE.md](ARCHITECTURE.md)**. Built on branches; compared empirically before cutover. | AL-2, AL-3 |
