...
- **Role-admin hierarchy (AL-8) — OZ `AccessControl` parity:** implemented and green.
  ...
  **Remaining (separate branch-pinned sub-PRs, non-blocking):** daml-verify rows
  (grant-requires-`roleAdmin`, no escalation, renounce self-only, timelock not
  bypassable) + a daml-props role-admin-graph state machine.
...
| **AL-10** | Account-level freeze / compliance hold | 📋 (ready; OQs resolved) | AL-2, AL-9 | **Must-Have** ([AL-9 §3](AL-9-PARITY-GAP-REVIEW.md#al-10--account-level-freeze--compliance-hold-must-have)). OFAC/AML/GENIUS-Act per-holder compliance hold, distinct from AL-1 origination pause. Issuer-signatory **`ComplianceAdmin`**-gated freeze/unfreeze (single officer — reversible); transfers refused by withheld signature (no public blocklist). **Manual-only** (OQ-2); audit metadata on every action. |
...
remain as separate branch-pinned sub-PRs (non-blocking).
...
|---|---|---|---|
| **1** | **AL-8** — full role delegability via a role-admin hierarchy (OZ parity) | **Core landed (green); verify/props pending.** Replaced the flat `delegableRole : Role -> Bool` with a **compiled** per-role `roleAdmin : Role -> Role` relation (decided §17.7: pure function, not a runtime `setRoleAdmin` contract) so **every role is delegable through its admin role**. `oz-access-control` gained mechanism-only role-admin-aware grant/revoke (`RoleAdmin_GrantRoleAs`/`RevokeRoleAs`) + `renounceRole` (`RoleGrant_Renounce`) + a self-contained timelocked two-step default-admin handoff (`DefaultAdminTransferOffer`); the token consumes them, keeps `Admin` **root-managed** (`PendingDefaultAdminTransfer` handoff; `AccessControlDefaultAdminRules` posture, C9) and adds self-only `RoleCapability_Renounce`. Suite 146/146; library +8 scripts; lint clean on new source. **Remaining (separate branch-pinned sub-PRs, non-blocking):** verify/props rows. Decisions in [§17.7](#177-resolved-decisions--al-8--al-9). | AL-7 |
...
| **2a** | **AL-10 / AL-11** — account freeze + forced transfer/seize | **📋 Ready (OQs resolved 2026-06-07).** Build with **manual-only** controls, new **`ComplianceAdmin`/`LegalAdmin`/`EmergencyOps`** roles (flat `roleAdmin`), and **two-person control for irreversible seize/forced-transfer** (OQ-2/OQ-4). | AL-9 |
