...
| freeze (AL-10) | a frozen sender/receiver cannot originate (unified `assertCanOriginate`); pause dominates; the admin is never freezable; freeze change is non-idempotent | `daml-verify` **PROVED** (A14–A18, [OpenZeppelin/daml-verify#6](https://github.com/OpenZeppelin/daml-verify/pull/6); `isFrozen` uninterpreted) + `daml-props` `Freeze` state-machine ([OpenZeppelin/daml-props#4](https://github.com/OpenZeppelin/daml-props/pull/4)); both stacked on the AL-8 branches |
