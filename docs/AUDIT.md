...
| INV-40 | A frozen account (AL-10 compliance hold) is refused at every origination chokepoint — as sender, receiver, or mint recipient (V1/V2 transfer, V1/V2 allocation, batch settlement, mint) | Script: `test_freezeBlocksTransferSender`, `test_freezeBlocksTransferReceiver`, `test_freezeBlocksAllocation`, `test_freezeBlocksMint`, `test_unfreezeRestoresTransfer` |
| INV-41 | Freeze/unfreeze require a **registry-wide** `ComplianceAdmin` capability (instrument-scoped ≠ registry-wide op; manual, capability-gated) | Script: `test_freezeRequiresComplianceAdmin`, `test_scopedComplianceAdminCannotFreeze`, `test_complianceAdminDelegableAndObservable` |
| INV-42 | Freeze is non-idempotent (already-frozen / not-frozen rejected) and the registry administrator can never be frozen | Script: `test_freezeNonIdempotentAndAdminProtected` |
