...
| INV-43 | A forced seizure (AL-11) is gated by **two-person control with separation of duties**: one `LegalAdmin` proposes (`Admin_ProposeSeizure`) and a DIFFERENT `LegalAdmin` approves (`SeizureProposal_Approve`, `approver /= proposer`); both steps require a `LegalAdmin` capability (instrument-scopable), and **neither officer may be the beneficiary** (`newOwner /= proposer` ∧ `/= approver`). The seize archive (`SimpleHolding_ForcedSeize`) is reachable only with the registry root's authority, so a lone capability holder cannot seize; cross-registry seizure is rejected (`eSeizureForeignHolding`). | Script: `test_seizeRequiresTwoOfficers`, `test_seizeProposeRequiresLegalAdmin`, `test_seizeApproveRequiresLegalAdmin`, `test_seizeBeneficiaryCannotBeOfficer`, `test_seizeForeignHoldingRejected`, `test_seizeScopedLegalAdmin` |
| INV-44 | Seize-and-reallocate preserves value: the seized holding is archived and an EQUAL holding (same amount + instrument) is created for the proposed `newOwner` via their preapproval — only the owner changes, net supply is unchanged, and the approver cannot redirect the value to a different party or registry. A stale target (owner already moved/burned it) fails `NotActive` and must be re-proposed. | Script: `test_seizeReallocatesToNewOwner`, `test_seizeCannotRedirect`, `test_seizeForeignPreapprovalRejected`, `test_seizeStaleTargetRejected`, `test_seizeCancellable` |
...
| seizure (AL-11) | two-person control + separation of duties (proposer ≠ approver, neither is the beneficiary, both LegalAdmin); value preserved on reallocation; approver cannot redirect or cross registries; stale target rejected | **daml-script** (`Test/Seize.daml`, 11 tests). **daml-verify / daml-props rows pending** (next branch-pinned sub-PRs, mirroring AL-8/AL-10: a two-person/SoD proof + a seizure property model). |
