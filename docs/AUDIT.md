...
| INV-43 | A forced seizure (AL-11) is gated by **two-person control**: one `LegalAdmin` proposes (`Admin_ProposeSeizure`) and a DIFFERENT `LegalAdmin` approves (`SeizureProposal_Approve`, `approver /= proposer`); both steps require a `LegalAdmin` capability. The seize archive (`SimpleHolding_ForcedSeize`) is reachable only with the registry root's authority, so a lone capability holder cannot seize. | Script: `test_seizeRequiresTwoOfficers`, `test_seizeProposeRequiresLegalAdmin`, `test_seizeApproveRequiresLegalAdmin` |
| INV-44 | Seize-and-reallocate preserves value: the seized holding is archived and an EQUAL holding (same amount + instrument) is created for the proposed `newOwner` via their preapproval — only the owner changes, net supply is unchanged, and the approver cannot redirect the value to a different party. A stale target (owner already moved/burned it) fails `NotActive` and must be re-proposed. | Script: `test_seizeReallocatesToNewOwner`, `test_seizeCannotRedirect`, `test_seizeStaleTargetRejected`, `test_seizeCancellable` |
...
| seizure (AL-11) | two-person control (proposer ≠ approver, both LegalAdmin); value preserved on reallocation; approver cannot redirect; stale target rejected | **daml-script** (`Test/Seize.daml`, 7 tests). **daml-verify / daml-props rows pending** (next branch-pinned sub-PRs, mirroring AL-8/AL-10: a two-person-control proof + a seizure property model). |
